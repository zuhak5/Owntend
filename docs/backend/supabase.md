...
`get_charged_operation_status(p_operation_id uuid, p_request_hash text)` enables client journal reconciliation following transport failure. Lookups are strictly same-account and operation-bound; querying another user's operation returns `status: 'not_found'` without revealing operation existence, while reuse with a mismatched request hash is rejected. The public function executes as `SECURITY DEFINER` with a pinned empty `search_path` and rejects execution when `auth.uid()` is null; its elevated journal lookup lives in `owntend_monetization_private` and still derives the caller from `auth.uid()`.

The media RPCs `prepare_asset_photo_upload`, `finalize_asset_photo_upload`, and `set_primary_asset_photo` follow the same boundary. Their public functions execute as `SECURITY DEFINER` with pinned empty `search_path`, reject anonymous callers through explicit `auth.uid()` null guards, and validate ownership internally while privileged implementations live in the non-exposed `owntend_media_private` schema. This keeps server-authoritative mutation centralized behind audited, pgTAP-enforced boundaries (`supabase/tests/database/0023_api_security.test.sql`) without granting clients any direct privilege on private implementations.
