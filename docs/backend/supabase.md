...
After a hosted pre-launch reset, the feed may be enabled only after the exact-main baseline is proven hosted: no pending migration drift, protocol `1.0.0`, zero malformed feed rows, correct effective RPC ACLs, no unresolved feed-related security Advisor findings, and parity success for every hosted account (or verified zero accounts).
