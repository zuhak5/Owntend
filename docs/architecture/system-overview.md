...
`OwntendProcessSplash` is the first child passed to every `runApp` branch. It remains mounted while deferred initialization, startup-theme loading, the application, or a startup-failure surface changes beneath it, so those branches cannot expose a blank Flutter frame. Splash dismissal is readiness-gated: when application bootstrap finishes and the initial screen is ready (`unauthenticated`, `authenticatedReady`, or `authenticatedHydrating`), dismissal triggers after a minimum 600ms brand settling duration (0ms when the platform requests reduced motion), while a 3200ms fallback timer ensures the splash never hangs if readiness signalling is delayed. Fatal startup failures signal immediate 0ms dismissal so error diagnostics and retry options are presented instantly. A static startup surface remains underneath if initialization outlives the overlay. The splash selects English or Arabic from the user's stored preference or device locale, exposes one localized semantic label, supports compact and scaled layouts, and stops repeating animation when the platform requests reduced motion. These are repository and widget-test contracts; launch behavior on a physical release device remains separate evidence.

Deferred startup treats Supabase client initialization as a required cloud boundary with offline fallback for established users. A cloud initialization failure is logged through the privacy-scrubbed logger; if the local database contains an existing bound account or non-pristine domain data, startup safely proceeds in offline mode so users retain local access to their data. For pristine unauthenticated states without local data, startup presents the localized cloud-unavailable surface with an explicit retry. Foreground restore services on Android run strictly during active running restores (`isRunning`), controlled directly by `StartupBootstrapController` via Riverpod providers without coupling service lifecycles to widget mount lifecycles. Authentication stream failures likewise remain Riverpod `AsyncError` states while consumers may retain the repository's cached session, so transport failure cannot masquerade as sign-out.
