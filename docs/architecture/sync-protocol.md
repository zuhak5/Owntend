...
- **Semantic Equality Comparison**: When verifying whether an existing cloud record satisfies a replayed creation (`_reconcileBatchCreationReplay`) or duplicate insert, the coordinator compares `semanticDataColumns` (domain attributes) rather than automatic transport/metadata timestamps (`created_at`, `updated_at`).
- **Auto-Convergent Alert Entities**: Transient alert items (`notification_inbox`) follow an automated convergence contract: creation matches satisfy local intent; read state is unioned (`read_at != null` wins); and remote deletions obsolete pending read updates. They never generate manual user-facing conflicts. Legacy or errant notification conflicts are auto-healed on startup and reconciliation.
...
- **User Resolution Surface**: Account links to `/sync-health`, which reads `FailedSyncMutationSummary` and enriched `SyncConflictSummary` values (providing human-readable entity title/name and formatted version timestamps for intelligible decision-making). Failed-visible work offers retry or confirmed dismissal. Unresolved conflicts offer confirmed keep-local or keep-cloud choices and pass the current bound account and device identity to `resolveSyncConflict`; a foreign or stale conflict cannot be resolved. The UI never exposes record keys, serialized preimages, account IDs, or raw error text.
...
- **Transactional Primary Photo Selection & Unique Constraint**: Primary asset photo selection uses the owner-authoritative `set_primary_asset_photo` RPC. The RPC performs a deterministic two-phase update within a single transaction: first demoting any existing primary photos (`is_primary = false`) for that asset, then promoting the candidate photo (`is_primary = true`). This prevents intra-statement uniqueness violations under non-deferrable partial unique index `idx_asset_photos_single_primary` on `asset_photos(user_id, asset_id) WHERE is_primary = true` regardless of PostgreSQL row-update evaluation order.
