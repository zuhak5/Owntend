...
- Capability discovery (`get_sync_feed_capability()`) allows server-controlled feature flagging and instant withdrawal without destroying historical change log entries. The checked-in v2 capability remains disabled; enabling it requires separate hosted compatibility and parity evidence.
