...
This document describes the behavior present in the repository as of 2026-08-15. The executable sources are:
...
When the server cannot confirm a charged operation, the client must not present it as completed or enqueue a blind wallet mutation. User input may be preserved only as an explicitly unfinished draft in a workflow that supports it. Every creation operation uses a durable pre-RPC journal write (`TaskCreationOperationStore` / `ChargedOperationJournal`) before invoking backend RPCs. If durable write to secure storage fails, the RPC is not executed. Ambiguous operations (`outcomeUnknown`) are resolved on startup and controller entry by `ChargedOperationResolver` using `get_charged_operation_status(p_operation_id, p_request_hash)` (capability version `1.1.0`). If the backend confirms `not_found` under capability `1.1.0`, the operation is resubmitted using the exact same operation ID (no replacement UUID is minted). Upon reaching terminal states (`reconciled` or `permanentRejected`), user-entered payloads are purged (`purgeTerminalPayloads`) to prevent unbounded content retention.
