...
Free copies use `copy_asset`. The server reads an active source asset owned by the caller, derives the asset/details/tasks/metadata itself, requires an exact map of all enabled unarchived source plans when tasks are included, and records zero-cost `asset_copy` entitlements. It never copies Storage objects. Tags and requested photos remain local/synchronization work; local-only photo intent is retained in the secure operation journal and is excluded from the RPC payload.
