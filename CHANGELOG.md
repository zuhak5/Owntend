...
- Implemented comprehensive Startup and Restoring Flow forensic remediation, platform lifecycle hardening, and accessibility overhaul (STARTUP-001 through STARTUP-016):
  - **Foreground Restore Service Liveness Boundary (`STARTUP-001`)**: Added `isRunning` (`state == RestoreRunState.running`) to `InitialHydrationProgress` and updated `startRestoreForegroundService` and `_RestoreTaskHandler._run()` to gate service execution strictly on running restore operations, while preserving `isActive` (`state != RestoreRunState.completed`) for `SyncStatus.initialHydrationProgress` so failed restore states and retry affordances remain visible to users.
  - **Foreground Restore Service Lifecycle & Race Elimination (`STARTUP-001b`)**: Added `progress` parameter to `startRestoreForegroundService` allowing immediate service startup upon entering `authenticatedHydrating` before SQLite writes complete. Serialized and debounced foreground service invocations in `StartupBootstrapController` to ensure monotonic progress updates. Captured starter and stopper functions upon controller construction, eliminating `UnmountedRefException` during provider disposal.
  - **Background Restore Task Redundant Sync Elimination (`STARTUP-001c`)**: Added `_readRestoreStatus()` with `hasActiveLease` check in `_RestoreTaskHandler` to prevent spinning up redundant background sync isolates and failing lease acquisition when the main isolate actively holds the sync lease.
  - **Offline Startup Fallback (`STARTUP-002`)**: Added non-blocking offline fallback during Supabase client initialization in `_initializeAfterFirstFrame()`. If cloud initialization fails but the local database has a bound user or is non-pristine, startup proceeds in offline mode rather than halting behind a blocking failure screen.
  - **Hydration Unit Accumulation Prevention (`STARTUP-003`)**: Added `resetCompleted: true` support to `AccountStore.setHydrationPlan()` and wired `RunCoordinator._pullAuthoritativeSnapshot()` to reset completed units when rebuilding hydration plans on retry, preventing completed unit counts from accumulating monotonically beyond total units.
  - **Cold-Launch Notification Deep Link Preservation (`STARTUP-004`)**: Updated `_goHomeAfterReadyIfRequested()` to always consume `PendingNotificationRoute.take()` upon readiness, ensuring notifications tapped during cold startup correctly route to the notification target regardless of whether an existing session was present or cold-restored.
  - **Immediate Notification Deep Link Route Cleanup (`STARTUP-004b`)**: Ensured `openNotificationPayload` consumes `PendingNotificationRoute.take()` when immediately pushing routes to mounted contexts, preventing stale pending routes from surviving in static memory.
  - **Safe Startup Sign-Out Lifecycle (`STARTUP-005`)**: Added explicit `syncCoordinator.suspend()` and awaited active bootstrap completion with a timeout before purging local session data in `signOutFromStartup()`, eliminating concurrency races between in-flight startup tasks and sign-out cleanup.
  - **Domain-Driven Foreground Restore Service (`STARTUP-006`)**: Lifted restore service invocation out of the presentation layer (`_StartupRestorationScreen`) into `StartupBootstrapController` via `startupRestoreServiceStarterProvider` and `startupRestoreServiceStopperProvider`, converting `_StartupRestorationScreen` into a pure stateless widget.
  - **Hydration Overlay Accessibility (`STARTUP-007`)**: Added `liveRegion: true` semantics to `_HydrationProgress`, programmatic screen reader announcements on error state transitions and retry taps via `SemanticsService.announce`, and enforced 48dp minimum touch target sizing across all failure action buttons.
  - **Hydration Overlay Dark Mode & RTL Support (`STARTUP-008`)**: Eliminated all hardcoded light-theme color constants from `hydration_overlay.dart`, transitioning to dynamic `Theme.of(context).colorScheme` (`onSurface`, `onSurfaceVariant`, `primary`), adaptive surface backgrounds and radial gradients, and mirrored directional icon transforms for RTL layouts.
  - **Decoupled Non-Blocking Avatar Warm-Up (`STARTUP-009`)**: Updated `_resolveStartupAvatar()` to warm local disk and cached images synchronously while decoupling remote HTTP downloads via `unawaited()`, eliminating network delays on the startup critical path.
  - **Canonical Device Locale Detection (`STARTUP-011`)**: Replaced ad-hoc language string comparisons in `_RestoreRecoveryGate` with canonical `supportedDeviceLanguage()`, ensuring consistent English/Arabic locale resolution across startup gates and foreground notification services.
  - **Startup Journal Recovery Wiring (`STARTUP-016`)**: Added comprehensive widget and unit tests verifying that pending restore operations and journals are resolved immediately upon startup bootstrap.

- Implemented comprehensive Restore experience forensic remediation, security boundary enforcement, and UX overhaul (RESTORE-001 through RESTORE-018):
  - **Pre-Restore Synchronization Barrier (`RESTORE-001`)**: Implemented `CloudSyncRepository.suspend()` and wired `onBeforeRestoreBarrier` in `app_providers.dart` to invalidate active sync epochs, cancel scheduled retry timers, stop realtime listeners, cancel background sync tasks, and clear scheduled reminders before SQLite import transactions begin, eliminating race conditions and corrupted sync states.
  - **In-Flight Navigation Protection (`RESTORE-002`)**: Wrapped `BackupScreen` in `PopScope(canPop: !_busy)` to prevent accidental back navigation or route pops while backup creation or restoration is actively executing.
  - **Live Restore Phase Feedback (`RESTORE-003`, `RESTORE-004`)**: Added `RestorePhase` enum across core domain contracts (`validated`, `safetyBackupComplete`, `servicesSuspended`, `mediaStaged`, `dbCommitStarted`, `dbCommitComplete`, `mediaActivated`, `cloudIntentDurable`, `terminal`) with `onProgress` callbacks from `BackupService` to presentation, surfacing real-time localized progress indicators ("Creating safety backup...", "Restoring local data...", "Restoring media files...", "Finalizing restore...").
  - **Exact Actionable Error Reporting (`RESTORE-005`)**: Updated `failureMessage` in `presentation_formatters.dart` to surface exact error messages from `BackupException` and localized prompts from `BackupPassphraseRequiredException` rather than collapsing all failures into a generic error string.
  - **Backup Manifest Content Localization (`RESTORE-006`)**: Updated `_localizedBackupDetail` to recognize `'Items, rooms, areas, tags, and photos'` alongside categories-inclusive manifests, preventing standard backups from falsely displaying compatibility warnings.
  - **Passphrase Unlock Ergonomics & Inline Retry (`RESTORE-007`, `RESTORE-008`)**: Added password visibility toggles, clear action labeling ("Unlock backup"), and inline error feedback in `_promptForRestorePassphrase` so users can re-enter their passphrase without needing to re-pick the backup container from storage. Preserved preview panel state upon cancellation of confirmation dialog.
  - **Cloud Snapshot Resumption Affordances (`RESTORE-009`)**: Added dedicated `_ResumeCloudUploadCard` on `BackupScreen` and `_SyncHealthResumeCloudCard` on `SyncHealthScreen` when `syncStatus.restorePending` is true, enabling users who performed a local-only restore to seamlessly upload their restored snapshot to the cloud via `resumeRestoredSnapshotToCloud()`.
  - **Clean Architecture & Technical Debt Elimination**: Deduped `RestorePhase`, `BackupException`, and `BackupPassphraseRequiredException` into core domain contracts (`contracts.dart`), eliminating duplicate declarations across services.

- Implemented comprehensive Home screen forensic remediation, performance hardening, and accessibility refactor (HOME-001 through HOME-018):
  - **Overdue Task Prioritization & Direct Filtering**: Added a dedicated top-priority Overdue task section rendered prominently above Today's and Tomorrow's tasks whenever overdue items exist. Attached direct-filter routing (`/maintenance?filter=overdue`) to the "See all" header action, resolving navigation ambiguity.
  - **Temporal Task Status Normalization**: Updated `getTaskBuckets` to dynamically normalize task statuses against current wall time using active due dates, preventing stale database statuses from leaking across midnight rollover or app sleep.
  - **N+1 Database Query Elimination**: Replaced full-table `select(db.rooms).get()` scans during task hydration in `MaintenanceRepository._hydrateTasks` with targeted `room.id.isIn(roomIds)` queries.
  - **Cumulative Layout Shift (CLS) Prevention**: Moved `HkNativeAdCard` below task lists and empty-state surfaces so dynamic ad loading never displaces interactive task cards.
  - **Scroll Position Restoration**: Added `PageStorageKey('home-scroll-storage')` to the primary dashboard `CustomScrollView`, maintaining viewport position across navigation tab switches.
  - **Coordinated Parallel Pull-to-Refresh**: Added a unified `RefreshIndicator` callback coordinating streak calculation, weather refresh, and cloud synchronization concurrently without nested reloads or redundant loops.
  - **Screen Reader & Gesture Semantics**: Replaced monolithic card `button: true` semantics on swipe-to-delete rows with targeted `CustomSemanticsAction` descriptors, preventing screen readers from announcing swipe action containers as plain buttons.
  - **Empty State & Readiness Reconciliation**: Eliminated the contradiction where a user with 100% readiness and completed tasks was presented with "No maintenance plans yet"; now properly displays "Your maintenance plan is up to date" and "Your maintenance plan is clear for today" while suppressing redundant floating action buttons during empty-state hero CTA display.
  - **Overdue Task Presentation & Count Headers**: Expanded the overdue task display limit to 5 items on Home and added localized total-count subtitles (`taskCountLabel`) to section headers when additional items exist beyond the home display limit.
  - **Pull-to-Refresh Concurrency & Domain Error Recovery**: Added an active refresh lock to deduplicate concurrent pull gestures and ensured errored local domain providers (`tasksProvider`, `assetsProvider`, `roomsProvider`) are invalidated for retry during pull-to-refresh.
  - **Fine-Grained Permission Education Watching**: Removed broad `permissionEducationControllerProvider` watching from `DashboardScreen.build`, scoping the subscription via `.select((s) => s.setupSnapshot?.weather)` inside `_DashboardWeatherCard` to prevent cascading dashboard rebuilds on overlay step transitions.
  - **BiDi RTL Isolation & Weather Freshness Precision**: Wrapped weather metric numerals in `bidiIsolate` to prevent numeral/percentage flipping in Arabic RTL, and updated weather timestamp formatting to show `formatShortDateTime` for multi-day stale cache to prevent confusing previous days with today's readings.
  - **Dead Code & Architectural Simplification**: Removed unused legacy `dashboardProvider`, eliminated redundant inner `RepaintBoundary` wrappers from weather and readiness cards, and pruned obsolete education link callbacks from header delegates.


- Implemented comprehensive splash and startup lifecycle revamp across Android native, engine startup, Flutter splash overlay, and theme/locale continuity:
  - **Readiness-Gated Splash Dismissal (Model B)**: Replaced the fixed 3200ms timer with readiness-gated dismissal coordinated via `hkStartupReadyNotifier` and `hkStartupFailedNotifier`. The animated splash dismisses when initial bootstrap converges (`unauthenticated`, `authenticatedReady`, or `authenticatedHydrating`), preserving a 600ms minimum display duration for brand settling while eliminating artificial waiting on fast warm/cold starts.
  - **Reduced Motion & Instant Error Dismissal**: When the platform requests reduced motion (`disableAnimations: true`), minimum presentation duration drops to 0ms to immediately dismiss upon readiness. On fatal bootstrap errors, the splash dismisses immediately with 0ms delay so users see error messaging and recovery actions without waiting.
  - **First-Frame Native-to-Flutter Visual Continuity**: Initialized splash logo opacity and scale to 1.0 at first Flutter frame, eliminating the jarring blink where the centered native Android splash icon disappeared only to fade in from opacity 0 / scale 0.72.
  - **Android Native Dark Splash Flash Resolution**: Added `drawable-night/launch_background.xml` and `drawable-night-v21/launch_background.xml` layer-lists referencing `#0D2118` (`splash_background`) and aligned `values-night-v31/styles.xml` parent theme to `@android:style/Theme.Material.NoActionBar`, completely eliminating the bright white screen flash during cold dark-mode launch on Android 8.0–11.
  - **Early Theme & Locale Hydration**: Added early SQLite settings query before `_runOwntendProcess` to mount `OwntendProcessSplash` with the user's stored theme brightness and locale, preventing theme and text-direction pops before deferred bootstrap finishes.
  - **Pre-Ready Scaffold Background Continuity**: Updated `StartupHome` placeholder scaffolds to use `Theme.of(context).scaffoldBackgroundColor` rather than hardcoded light background colors.
  - **GPU & Layout Performance**: Added `RepaintBoundary` to animated background painter and logo icon, reduced heavy `BoxShadow` blur radius, positioned sparkle coordinates with absolute canvas coordinates to prevent asymmetric distortion in RTL, and added flex overflow scroll protection on compact screens.
  - **Indeterminate Progress & Accessibility**: Replaced the deceptive determinate progress bar with a three-dot pulsing indeterminate indicator and added `liveRegion: true` semantics.
  - Added `owntendSplashBackgroundDark` (`#0D2118`) as canonical constant across Flutter and Android resource definitions.
...
- Resolved 40 UI, accessibility, and localization issues across the application:
  ...
  - Fixed Arabic RTL layout and typography bugs: directional padding and constraints in `EditorSheetFrame` and `AppPageHeader`, bidirectional Unicode text isolation (`bidiIsolate`) on formatted numerals, percentages, and geographical coordinates, mirrored illustration edge masks in `_EdgeFeatherMask`, and directional positioning (`PositionedDirectional`) across empty states, brand marks, task cards, swipe-to-delete actions, achievement cards, and onboarding hero sparkle.
  - Enhanced form input ergonomics and state feedback: added specialized keyboard types (`text`, `multiline`, `phone`, `number`), input actions (`next`, `done`, `newline`), text capitalization (`words`, `sentences`), and digit-only formatters across Area, Room, and Asset editors; added async loading indicators (`isSaving`) to `EditorSheetFrame` submit buttons; and added retry recovery callbacks to task error panels.
  - Re-baselined visual golden test suites for dashboard headers, Arabic onboarding, and notification surfaces.
- Completed a second exhaustive UI and accessibility audit across all application screens and dialogs:
  - Ensured all 15 `AlertDialog` surfaces throughout the app (`AssetDialogs`, `AccountScreen`, `BackupScreen`, `MaintenanceDialogs`, `TaskDisposalActions`, `EarnPointsFlow`, `PointShortageDialog`, `SettingsScreen`, `SyncHealthScreen`, `RoomDialogs`) are wrapped in `scrollable: true` and configured with `actionsOverflowButtonSpacing: HkSpacing.xs` to prevent viewport overflow and button clobbering on compact displays, split-screen views, landscape mode, and large font-scale settings.
  - Removed redundant nested `SingleChildScrollView` wrappers from inside `AlertDialog` content blocks in `BackupScreen` and `PointShortageDialog` to eliminate nested scrolling conflicts.
  - Refactored `task_actions.dart` snooze modal bottom sheet with `isScrollControlled: true` and `SingleChildScrollView` to prevent layout overflow in landscape orientation.
  - Added `Semantics(button: true)` and interactive `InkWell` ripple feedback to the inline complete action inside notification cards (`notifications_screen.dart`).
  - Corrected RTL mirroring for splash screen decorative sparkles (`owntend_animated_splash_screen.dart`) and state illustration accents (`state_feedback.dart`) using `PositionedDirectional`.
  - Unified theme color tokens across task burst animations and restore dialogs to use ambient `Theme.of(context).colorScheme` rather than hardcoded static palette constants.
  - Applied Unicode bidirectional text isolation (`bidiIsolate`) across wallet balance ratio (`wallet_sheet.dart`), initial cloud hydration progress percentage (`hydration_overlay.dart`), and statistics metrics (`statistics_screen.dart`).
  - Constrained dynamic location text in `task_status.dart` within a maximum width boundary to prevent single-line horizontal run overflow in `Wrap` containers on narrow viewports.
  - Pruned unreachable dead legacy layout code from `_OnboardingActions.build` in `authentication_gate.dart` per repository lifecycle protocol.
- Fixed soft keyboard occlusion in editor bottom sheet modals (`AreaEditorDialog`, `RoomEditorDialog`, `AssetEditorDialog`, `PlanEditorDialog`, `CompleteTaskDialog`, `LocationPickerSheet`). Extracted `_EditorModalHost` into a reactive widget subscribed to `MediaQuery` to dynamically lift the modal sheet above the soft keyboard on input focus, accounted for status bar top padding in available height calculations, enabled keyboard drag dismiss on scroll, and routed all location picker invocations through `showEditorModal` (`[UI-01]`, `[BUG-13]`).
...
- Advanced the application build number to 11 (`1.0.1+11`) to ensure monotonic `versionCode` progression for Android update compatibility.
