  ...
  test(
    ...
    () async {
      ...
      expect(
        ...
        reason:
            'The screen avoids a duplicate delay before changing the '
  ...
  test('recent-sync resume performs a broad pull and repairs a missed remote change', () async {
    ...
    await _eventually(() async {
      final row = await db
          ...
          .getSingleOrNull();
      return gateway.feedCalls > 0 && row?.read<String>('value') == 'dark';
    });

    expect(gateway.feedCalls, greaterThan(0));
  });
}
...
class _RecordingSyncGateway extends SupabaseSyncGateway {
  ...
  Future<UserChangeFeedPage> fetchUserChangeFeed({
    ...
    final includeCandidate = candidate != null && sinceSeq < 1;
    final highWater = candidate == null ? sinceSeq : 1;
    return UserChangeFeedPage(
      entries: includeCandidate
          ? [
              ChangeFeedEntry(
                changeSeq: 1,
                record: candidate,
      ...
      nextSeq: includeCandidate ? highWater : sinceSeq,
      hasMore: false,
