...
void main() {
  test(
    'local domain mutations propagate through Drift providers without a loading gap',
    () async {
      ...
      await _eventually(() {
        return container.read(roomsProvider).value?.single.name == 'Utility' &&
            container.read(assetsProvider).value?.single.name ==
                'Water heater' &&
  ...
  test('repository settle coalescing emits only the coherent aggregate', () async {
    ...
    expect(emissions.single, const _AggregateSnapshot(room: 'old', asset: 'old'));
  ...
  test('Home and Rooms keep runtime domain authority in live providers', () async {
    ...
    final assetLiveRead = dashboard.indexOf('ref.watch(assetsProvider).value');
  ...
  test(
    'recent-sync resume performs a broad pull and repairs a missed remote change',
    () async {
      final db = AppDatabase(executor: NativeDatabase.memory());
      addTearDown(db.close);
...
void _expectNoPostPopulationGap<T>(
  ...
  for (final state in states.skip(populatedAt)) {
    ...
    expect(
      state.value,
      isNotEmpty,
      reason: '$label must not flash an empty collection after it is populated.',
    );
  }
}
