...
import { readZipEntries } from "./android_apk_size_report.mjs";
import {
...
async function downloadAsset(asset, token, destination) {
  ...
  if (total !== asset.size) throw new Error(`Downloaded APK size ${total} does not match GitHub asset size ${asset.size}.`);
...
async function findAndroidTool(name) {
  ...
  const versions = (await fsPromises.readdir(path.join(androidHome, "build-tools"), { withFileTypes: true }))
...
async function resolveReleaseCommit(tag, historicalDecision) {
  ...
  if (!historicalDecision) {
    throw new Error("Release commit is not an ancestor of current main and no explicit historical decision exists.");
  }
  if (historicalDecision.tag !== tag || String(historicalDecision.commitSha || "").toLowerCase() !== commitSha) {
    throw new Error("Historical release decision does not match the release tag and commit.");
  }
  return commitSha;
}
...
export function inspectApkAbis(buffer) {
  const abis = new Set();
  for (const entry of readZipEntries(buffer)) {
    const match = entry.path.match(/^lib\/([^/]+)\//);
    if (match) abis.add(match[1]);
  }
  return [...abis].sort();
}

function assertExpectedAbi(apkPath, expectedAbi) {
  const abis = inspectApkAbis(fs.readFileSync(apkPath));
  if (expectedAbi === "universal") {
    if (!abis.length || abis.some((abi) => !VERSIONDECK_SPLIT_ABIS.includes(abi))) {
      throw new Error(`Universal APK contains unexpected native ABIs: ${abis.join(", ") || "none"}.`);
    }
    return abis;
  }
  if (!VERSIONDECK_SPLIT_ABIS.includes(expectedAbi)) throw new Error(`Unsupported expected ABI ${expectedAbi}.`);
  if (abis.length !== 1 || abis[0] !== expectedAbi) {
    throw new Error(`APK ABI mismatch. Expected exactly ${expectedAbi}, found ${abis.join(", ") || "none"}.`);
  }
  return abis;
}
...
export async function verifyReleaseArtifact({
  ...
  try {
    ...
    const abis = assertExpectedAbi(apkPath, expectedAbi);
    ...
    const provenancePolicy = buildVersionDeckApkProvenancePolicy({ commitSha, apkAsset, sha256 });
...
export function assertExpectedRepository(repository) {
  ...
  if (VERSIONDECK_PACKAGE_NAME !== "app.owntend.mobile") throw new Error("VersionDeck package policy is invalid.");
}
