...
select extensions.has_function('public', 'get_sync_feed_capability', ARRAY[]::text[], 'capability RPC exists');
select extensions.has_function('public', 'fetch_user_change_feed', ARRAY['bigint', 'integer'], 'feed pull RPC exists');
select extensions.has_function('public', 'validate_change_feed_parity', ARRAY[]::text[], 'owner-scoped parity RPC exists');
...
select extensions.ok(
  not (select p.prosecdef from pg_proc p where p.oid = 'public.get_sync_feed_capability()'::regprocedure),
  'get_sync_feed_capability is SECURITY INVOKER'
);
select extensions.ok(
...
select extensions.ok(
  (select p.prosecdef from pg_proc p where p.oid = 'public.fn_log_server_change_feed()'::regprocedure),
...
select extensions.ok(
  not has_function_privilege('anon', 'public.get_sync_feed_capability()', 'execute'),
  'anon cannot execute capability RPC'
);
select extensions.ok(
...
select extensions.ok(
  not has_function_privilege('anon', 'public.fn_log_server_change_feed()', 'execute'),
...
select extensions.ok(
  has_function_privilege('authenticated', 'public.get_sync_feed_capability()', 'execute'),
  'authenticated can execute capability RPC'
);
select extensions.ok(
...
select extensions.ok(
  has_function_privilege('authenticated', 'public.validate_change_feed_parity()', 'execute'),
  'authenticated can execute owner-scoped parity RPC'
);
select extensions.ok(
...
select extensions.ok(
  not has_function_privilege('authenticated', 'public.fn_log_server_change_feed()', 'execute'),
