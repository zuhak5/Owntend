...
select ok(
  exists (
    ...
    from pg_constraint
    where conrelid =
        'owntend_archive.profiles_legacy_media_20260720'::regclass
      and confrelid = 'auth.users'::regclass
      and confdeltype = 'c'
  ),
  'legacy profile media is deleted with the Auth user'
);
select ok(
  exists (
    select 1
    from pg_constraint
    where conrelid =
        'owntend_archive.asset_photo_upload_metadata_20260720'::regclass
      and confrelid = 'auth.users'::regclass
      and confdeltype = 'c'
  ),
  'legacy asset media metadata is deleted with the Auth user'
);
select ok(
  exists (
    select 1
...
select ok(
  ...
  'legacy profile media Auth foreign key is indexed'
);
select ok(
  to_regclass(
    'owntend_archive.asset_photo_upload_metadata_user_id_idx'
  ) is not null,
  'legacy asset media Auth foreign key is indexed'
);
select ok(
  to_regclass(
