-- Migration: 20260815000004_task_completion_and_rpc.sql
-- Description: Server-Authoritative Maintenance Task Completion RPC (Security Invoker, Atomic Transaction)
...
BEGIN
  ...
  IF record_completed_at IS NOT NULL AND record_completed_at > date_trunc('second', clock_timestamp()) THEN
...
REVOKE ALL ON FUNCTION public.complete_maintenance_task(JSONB, TEXT) FROM PUBLIC, anon, service_role;
GRANT EXECUTE ON FUNCTION public.complete_maintenance_task(JSONB, TEXT) TO authenticated;
